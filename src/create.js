...
window.addEventListener("keydown",(event) => {
    if(event.keyCode==32){
        ...
        if(starttime == ''){
            starttime = new Date();
            const p = new Audio('musics/syaryry.ogg');
            p.play();
        }
        else{
            //書き出し
            const blob = new Blob([JSON.stringify(Nodos)], {
...
function don(){
    console.log("don")
    timing = new Date().getTime() - starttime.getTime();
    Nodos.push({type:0,time:timing})
    new Nodo(0)
